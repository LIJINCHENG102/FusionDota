-- Terrorblade Metamorphosis技能
-- 变身持续10秒，变身后获得额外的400攻击距离与15%攻速加成，且每次普攻附带50点额外魔法伤害
-- 每2秒偷取场上所有单位100点生命值

terrorblade_metamorphosis = class({})

LinkLuaModifier("modifier_terrorblade_metamorphosis_custom", "heroes/hero_terrorblade/terrorblade_metamorphosis.lua", LUA_MODIFIER_MOTION_NONE)

-- 预加载模型资源
function terrorblade_metamorphosis:Precache(context)
    PrecacheResource("model", "models/heroes/terrorblade/demon.vmdl", context)
end

-- 确保技能没有冷却时间
function terrorblade_metamorphosis:GetCooldown(level)
    return 0
end

-- 自走棋式自动施法功能
function terrorblade_metamorphosis:OnCreated()
    if not IsServer() then return end
end

function terrorblade_metamorphosis:OnUpgrade()
    if not IsServer() then return end
    ...
    if not self.auto_cast_timer then
        ...
        local function CheckAutoCast()
            ...
            if current_mana >= max_mana and self:IsFullyCastable() then
                local has_metamorphosis = caster:HasModifier("modifier_terrorblade_metamorphosis_custom")
                if not has_metamorphosis then
        ...
        GameRules:GetGameModeEntity():SetThink(CheckAutoCast, "TerrorbladeMetamorphosisAutoCast_" .. caster:GetEntityIndex(), 0.1)
    end
end

function terrorblade_metamorphosis:OnSpellStart()
    local caster = self:GetCaster()
    ...
    -- 如果已经有变身效果，先移除
    if caster:HasModifier("modifier_terrorblade_metamorphosis_custom") then
        caster:RemoveModifierByName("modifier_terrorblade_metamorphosis_custom")
    end
    
    -- 添加变身效果（最简单的 modifier）
    caster:AddNewModifier(caster, self, "modifier_terrorblade_metamorphosis_custom", {duration = duration})
...
if modifier_terrorblade_metamorphosis_custom == nil then
...
function modifier_terrorblade_metamorphosis_custom:OnCreated(params)
    ...
    if IsServer() then
        ...
        if #self.hidden_wearables == 0 then
            GameRules:GetGameModeEntity():SetThink(function()
                ...
                if not caster:HasModifier("modifier_terrorblade_metamorphosis_custom") then return nil end
        ...
        -- 调试信息
        print("Terrorblade Metamorphosis: OnCreated")
...
function modifier_terrorblade_metamorphosis_custom:OnIntervalThink()
    ...
    if current_time - self.last_life_steal_time >= life_steal_interval then
        ...
        if total_stolen > 0 then
            ...
            -- 调试信息
            print("Terrorblade Metamorphosis: Life steal triggered, total_stolen:", total_stolen)
        end
    end
...
-- 提供攻击速度加成（使用安全的实现方式）
function modifier_terrorblade_metamorphosis_custom:GetModifierAttackSpeedBonus_Constant()
...
function modifier_terrorblade_metamorphosis_custom:GetModifierConstantManaRegen()
    return -9999
end
...
-- 处理攻击附加魔法伤害
function modifier_terrorblade_metamorphosis_custom:OnAttackLanded(params)
    if not IsServer() then return end
...
-- 阻止通过受到伤害获得的mana
function modifier_terrorblade_metamorphosis_custom:OnTakeDamage(params)
    if not IsServer() then return end
