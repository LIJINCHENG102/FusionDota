...
function modifier_double_cast:TryAutoCastAbility(unit, ability)
    ...
    elseif ability_name == "razor_eye_of_the_storm" then
        self:TryCastRazorEyeOfTheStorm(unit, ability)
    elseif ability_name == "terrorblade_metamorphosis" then
        self:TryCastTerrorbladeMetamorphosis(unit, ability)
...
function modifier_double_cast:TryCastRazorEyeOfTheStorm(unit, ability)
...
-- Terrorblade魔化技能的特殊双重施法逻辑
function modifier_double_cast:TryCastTerrorbladeMetamorphosis(unit, ability)
    -- 检查变身效果是否已经结束
    local has_metamorphosis = unit:HasModifier("modifier_terrorblade_metamorphosis_custom")
...
function modifier_double_cast:ScheduleDoubleCastAfterDuration(unit, ability, skill_key, duration)
    ...
    GameRules:GetGameModeEntity():SetThink(function()
        ...
        if ability_name == "razor_eye_of_the_storm" then
            -- 风暴之眼：检查技能是否可用（如果可用说明上一次效果已结束）
            skill_ended = ability:IsFullyCastable()
        elseif ability_name == "terrorblade_metamorphosis" then
            -- 魔化：检查变身效果是否已结束
            skill_ended = not unit:HasModifier("modifier_terrorblade_metamorphosis_custom")
        elseif ability_name == "dawnbreaker1_solar_guardian_land" then
            -- 天光现世：检查引导效果是否已结束
